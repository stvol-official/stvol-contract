...
contract(
  ...
  ([operator, admin, owner, overUser1, overUser2, overUser3, underUser1, underUser2, underUser3, participantVault, overLimitUser1, overLimitUser2, overLimitUser3, underLimitUser1, underLimitUser2, underLimitUser3]) => {
    ...
    it("Should participate with strike by user", async () => {
      ...
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("2.1"), { from: overUser1 }); // 2.1 USDC
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("2.2"), { from: overUser2 }); // 2.2 USDC
      await stVol.participateUnder(currentEpoch, STRIKE._100, ether("2.4"), { from: underUser1 }); // 2.4 USDC
      ...
      updateData = await pyth.createPriceFeedUpdateData(priceId, FIRST_PRICE, 10 * FIRST_PRICE, -5, FIRST_PRICE, 10 * FIRST_PRICE, currentTimestamp);
      requiredFee = await pyth.getUpdateFee([updateData]);
      ...
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("3.1"), { from: overUser1 }); // 3.1 USDC
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("3.2"), { from: overUser2 }); // 3.2 USDC
      await stVol.participateUnder(currentEpoch, STRIKE._100, ether("3.4"), { from: underUser1 }); // 3.4 USDC
      ...
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("4.1"), { from: overUser1 }); // 4.1 USDC
      await stVol.participateOver(currentEpoch, STRIKE._100, ether("4.2"), { from: overUser2 }); // 4.2 USDC
      await stVol.participateUnder(currentEpoch, STRIKE._100, ether("4.4"), { from: underUser1 }); // 4.4 USDC
    ...
    it.only("Should refund all user's participant amount when round is fail", async () => {
      ...
      await stVol.genesisOpenRound(currentTimestamp);
      currentEpoch = await stVol.currentEpoch();
      await stVol.participateUnder(currentEpoch, STRIKE._100, ether("10"), { from: underUser1 });

      // place limit order
      let limitOrderTx = await stVol.participateLimitOver(currentEpoch, STRIKE._100, ether("1"), new BN(2 * MULTIPLIER), prevIdx, { from: overLimitUser1 }); // payout:2x
      ...
      limitOrderTx = await stVol.participateLimitOver(currentEpoch, STRIKE._100, ether("1"), new BN(2 * MULTIPLIER), prevIdx, { from: overLimitUser1 }); // payout:2x
      ...
      limitOrderTx = await stVol.participateLimitUnder(currentEpoch, STRIKE._100, ether("1"), new BN(5 * MULTIPLIER), prevIdx, { from: underLimitUser1 }); // payout:5x
      expectedTimestamp = (await time.latest()).toNumber();
      expectEvent(limitOrderTx, "ParticipateLimitOrder", {
        idx: new BN(7),
