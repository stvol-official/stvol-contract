...
import { SuperVolFiveMinStorage } from "../storage/SuperVolFiveMinStorage.sol";
import { Round, Coupon, WithdrawalRequest, ProductRound, SettlementResult, WinPosition, OneMinOrder as FiveMinOrder, Position, ClosingOneMinOrder as ClosingFiveMinOrder, PriceInfo, PriceUpdateData } from "../types/Types.sol";
import { ISuperVolErrors } from "../errors/SuperVolErrors.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract SuperVolFiveMin is
  Initializable,
  UUPSUpgradeable,
  OwnableUpgradeable,
  PausableUpgradeable,
  ...
  ISuperVolErrors
{
  ...
  uint256 private constant ROUND_INTERVAL = 300; // 5 minutes
  uint256 private constant ROUND_DURATION = ROUND_INTERVAL * 2; // 10 minutes
  ...
  modifier onlyAdmin() {
    SuperVolFiveMinStorage.Layout storage $ = SuperVolFiveMinStorage.layout();
  ...
  function submitFiveMinOrders(FiveMinOrder[] calldata orders) external nonReentrant onlyOperator {
    SuperVolFiveMinStorage.Layout storage $ = SuperVolFiveMinStorage.layout();
    for (uint i = 0; i < orders.length; i++) {
      FiveMinOrder calldata order = orders[i];
      if ($.fiveMinOrders[order.idx].idx != 0) {
      ...
      $.fiveMinOrders[order.idx] = order;
      emit DebugLog(string.concat("Order ", Strings.toString(order.idx), " added"));
    }
  }

  function closeFiveMinOrders(ClosingFiveMinOrder[] calldata closingOrders) public onlyOperator {
    SuperVolFiveMinStorage.Layout storage $ = SuperVolFiveMinStorage.layout();
    for (uint i = 0; i < closingOrders.length; i++) {
      ClosingFiveMinOrder calldata closingOrder = closingOrders[i];

      FiveMinOrder storage order = $.fiveMinOrders[closingOrder.idx];
      if (order.idx == 0) {
        emit DebugLog(
  ...
  function settleFiveMinOrders(
    ...
    for (uint i = 0; i < orderIds.length; i++) {
      ...
      FiveMinOrder storage order = $.fiveMinOrders[orderIds[i]];
  ...
  function closeFiveMinOrder(uint256 idx, uint256 price) public onlyOperator {
    SuperVolFiveMinStorage.Layout storage $ = SuperVolFiveMinStorage.layout();
    FiveMinOrder storage order = $.fiveMinOrders[idx];
  ...
  function fiveMinOrders(uint256[] calldata idx) public view returns (FiveMinOrder[] memory) {
    SuperVolFiveMinStorage.Layout storage $ = SuperVolFiveMinStorage.layout();
    FiveMinOrder[] memory orders = new FiveMinOrder[](idx.length);
    for (uint i = 0; i < idx.length; i++) {
      orders[i] = $.fiveMinOrders[idx[i]];
    }
    return orders;
  }
