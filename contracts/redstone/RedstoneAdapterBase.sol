...
abstract contract RedstoneAdapterBase is RedstoneConsumerNumericBase, IRedstoneAdapter {
   ...
   */
  function updateDataFeedsValues(uint256 dataPackagesTimestamp) public override {
   ...
   */
  function validateProposedDataPackagesTimestamp(uint256 dataPackagesTimestamp) public view override {
  ...
  function validateDataPackagesTimestampOnce(uint256 dataPackagesTimestamp) public view virtual {
    ...
    (
      uint256 maxDataAheadSeconds,
      uint256 maxDataDelaySeconds
   ...
   */
  function getBlockTimestampFromLatestUpdate() public view override returns (uint256 blockTimestamp) {
   ...
   */
  function _unpackTimestamps(
    uint256 packedTimestamps
  ) internal pure returns (uint128 dataTimestamp, uint128 blockTimestamp) {
   ...
   */
  function getValueForDataFeed(bytes32 dataFeedId) public view override returns (uint256) {
   ...
   */
  function validateDataFeedValueOnRead(
    bytes32 dataFeedId,
    uint256 valueForDataFeed
  ) public view virtual {
   ...
   * @param valueForDataFeed Proposed value for the data feed
   */
  function validateDataFeedValueOnWrite(
    bytes32 dataFeedId,
    uint256 valueForDataFeed
  ) public view virtual {
    if (valueForDataFeed == 0) {
   ...
   */
  function getValuesForDataFeedsUnsafe(
    bytes32[] memory requestedDataFeedIds
  ) public view virtual returns (uint256[] memory values) {
